...
describe('ActivitiFilters', function () {
    ...
    fakeGlobalFilter.push(new ng2_activiti_tasklist_1.FilterRepresentationModel({ name: 'FakeInvolvedTasks', filter: { state: 'open', assignment: 'fake-involved' } }));
    fakeGlobalFilter.push(new ng2_activiti_tasklist_1.FilterRepresentationModel({ name: 'FakeMyTasks', filter: { state: 'open', assignment: 'fake-assignee' } }));
    ...
    beforeEach(function () {
        ...
        activitiService = new activiti_process_service_1.ActivitiProcessService(null);
        filterList = new activiti_filters_component_1.ActivitiProcessFilters(null, activitiService, logService);
    });
    it('should return the filter task list', function (done) {
        spyOn(activitiService, 'getProcessFilters').and.returnValue(Rx_1.Observable.fromPromise(fakeGlobalFilterPromise));
    ...
    it('should emit an error with a bad response', function (done) {
        filterList.appId = 1;
        spyOn(activitiService, 'getProcessFilters').and.returnValue(Rx_1.Observable.fromPromise(fakeErrorFilterPromise));
    ...
    it('should emit an event when a filter is selected', function (done) {
        var currentFilter = new ng2_activiti_tasklist_1.FilterRepresentationModel({ filter: { state: 'open', assignment: 'fake-involved' } });
    ...
    it('should return the current filter after one is selected', function () {
        var filter = new ng2_activiti_tasklist_1.FilterRepresentationModel({ name: 'FakeMyTasks', filter: { state: 'open', assignment: 'fake-assignee' } });
